...
function contentTypeForExt(ext) {
  return ({ mp4: "video/mp4", webm: "video/webm", m4a: "audio/mp4", mp3: "audio/mpeg" })[String(ext).toLowerCase()] || "application/octet-stream";
}
...
async function proxyDirect(rawUrl, res) {
  const upstream = await fetch(rawUrl, { method: "GET", redirect: "follow", headers: { "User-Agent": UA }, signal: AbortSignal.timeout(45000) });
  if (!upstream.ok) throw new Error(`Remote server returned ${upstream.status}.`);
  const len = Number(upstream.headers.get("content-length") || 0);
  if (len && len > MAX_BYTES) throw new Error("File is larger than the 500 MB limit.");
  const type = upstream.headers.get("content-type") || "application/octet-stream";
  const filename = safeName(path.basename(new URL(rawUrl).pathname) || "download");
...
async function runFfmpegToFile(inputUrl, outputPath, referer = "https://www.dailymotion.com/") {
